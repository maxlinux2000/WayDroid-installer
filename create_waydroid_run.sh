...
apt update
# Se añade YAD para el instalador de APK
...
echo "[2/9] Añadiendo el repositorio oficial de Waydroid (forzando '$WAYDROID_DISTRO_ARG')..."
...
if ! apt install python3-pyclipper -y; then
    echo "⚠️ Advertencia: Error al instalar python3-pyclipper. El portapapeles podría no funcionar."
fi
echo "✅ python3-pyclipper instalado (si estaba disponible)."
echo "---"
...
# -----------------------------------------------------------------------------
echo "[6/9] Creando script auxiliar de activación de Google Play..."
...
echo "----------------------------------------------------------------------"
echo "Después de registrar la ID y esperar unos minutos, reinicie Waydroid."
echo "Comando para reiniciar: waydroid session stop"
echo "----------------------------------------------------------------------"
echo ""
read -r -p "Presione [Enter] para finalizar la visualización de este mensaje."
exit 0
EOF_HELPER
...
# -----------------------------------------------------------------------------
echo "[7/9] Creando script de reinicio rápido de Waydroid..."
...
# -----------------------------------------------------------------------------
# 7. Creación del Script Instalador de APK (waydroid-apk-installer.sh)
# -----------------------------------------------------------------------------
...
# Abre un diálogo YAD para seleccionar un APK e instalarlo en Waydroid.

clear
APK_FILE=$(yad --center --title="Seleccionar archivo APK" --text="Seleccione el paquete de aplicación Android (.apk) que desea instalar en Waydroid." --file --class="WayDroidInstaller" --file-filter="Paquetes APK (*.apk)|*.apk" --width=600 --height=400)

# Comprobar si se seleccionó un archivo
if [ $? -ne 0 ] || [ -z "$APK_FILE" ]; then
    yad --center --title="Cancelado" --text="Instalación cancelada por el usuario." --button="Aceptar:0" --image="dialog-information"
    exit 0
fi
...
kill $PID # Detener el diálogo de progreso
wait $PID 2>/dev/null

if [ $EXIT_CODE -eq 0 ]; then
    yad --center --title="Instalación Exitosa" --text="El paquete APK ha sido instalado correctamente en Waydroid.\n\n$INSTALL_OUTPUT" --button="Aceptar:0" --image="dialog-ok"
else
    yad --center --title="Error de Instalación" --text="Hubo un error al instalar el paquete APK.\n\nSalida del error:\n$INSTALL_OUTPUT" --button="Aceptar:0" --image="dialog-error"
fi
...
echo "✅ Script instalador de APK $APK_INSTALLER_PATH creado."
echo "---"


# -----------------------------------------------------------------------------
# 8. Creación de Launchers (.desktop)
# -----------------------------------------------------------------------------
...
Comment=Instala aplicaciones descargadas desde archivos APK usando una interfaz gráfica simple.
...
echo "   Para la activación y uso de APKs, use los lanzadores después de reiniciar."
echo ""
